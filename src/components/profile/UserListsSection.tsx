...
export default function UserListsSection() {
  ...
  const { lists, loading } = useUserMovies();
  ...
  const formattedCount = (count: number) =>
    count === 0
      ? t("emptyCount", { defaultValue: "Aucun élément" })
      : count === 1
      ? t("singleCount", { count })
      : t("multiCount", { count });
    ...
    <section className="mt-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 sm:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {listCards.map((list, index) => {
          const count = counts[list.key as keyof typeof counts];
          ...
          return (
            <motion.div
              ...
              className={`relative flex flex-col justify-between rounded-2xl
                p-5 sm:p-6 bg-gradient-to-br ${list.gradient}
            ...
            >
              ...
              {/* --- Description --- */}
              <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-4 sm:mb-5 line-clamp-3">
                {list.description}
              </p>

              {/* --- Nombre d'éléments stylisé --- */}
              <div className="flex items-center justify-start mb-5">
                {loading ? (
                  <span className="inline-flex items-center gap-2 text-slate-400 text-sm">
                    ...
                    {t("loadingCount", { defaultValue: "Chargement..." })}
                  </span>
                ) : (
                  <span
                    className={`
      ...
      `}
              ...
              <button
                ...
                className={`relative flex items-center justify-center gap-2
                  ...
                  bg-gradient-to-r ${list.gradient} text-white border ${list.border}
